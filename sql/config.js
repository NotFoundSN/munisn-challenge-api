...
const db = mysql.createConnection({
	...
	host: "200.58.106.147",
	user: "muniChallenge",
    password: "muniPassword123",
	database: "muni_challenge",
	port: 3307,
});
	...
	usuario: {
		...
		search: async (dni) => {
			const [resultados] = await db
				.promise()
				.execute(`SELECT * FROM usuarios WHERE DNI=${dni}`); //busco todos los usuarios con ese dni
			if (resultados.length == 0) {
				//evaluo si obtuvo resultados
		...
		viewAll: async () => {
			const [resultados] = await db
				.promise()
				.execute(`SELECT * FROM usuarios`); //busco todos los usuarios
	...
	admin: {
		...
		exist: async (id, name, username) => {
			const [resultados] = await db
				.promise()
				.execute(
					`SELECT * FROM administradores WHERE id= ${id} AND usuario = '${username}' AND nombre = '${name}'`
				); //busco el admin
			if (resultados.length == 0) {
				//evaluo si obtuvo resultados
				return false; //devuelvo falso
